...
#'   coerced to double? Note that this function uses the character value from
#'   the factor, while `as.double()` uses the integer index of the factor.
#' @param min_value (length-1 `double`, `expression`, or `reactive expression`,
#'   or `NULL`) The lowest allowed value for `x`. If `NULL` (default) values are
#'   not checked.
#' @param max_value (length-1 `double`, `expression`, or `reactive expression`,
#'   or `NULL`) The highest allowed value for `x`. If `NULL` (default) values
#'   are not checked.
#' @inheritParams shared-params
#'
#' @returns A `vrv` object which returns a validated double vector.
#' @export
vrv_dbl <- function(
  value = NULL,
  default = NULL,
  ...
  vrv_from_function(
    validation_fn = stbl::stabilize_dbl,
    value = {{ value }},
    default = {{ default }},
    label = label,
    env = env,
...
vrv_dbl_scalar <- function(
  ...
  vrv_from_function(
    validation_fn = stbl::stabilize_dbl_scalar,
    value = {{ value }},
    default = {{ default }},
    label = label,
    env = env,
