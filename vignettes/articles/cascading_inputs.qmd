...
The `shinybatch` package provides `vrv_factor()` to solve this exact problem. It's a specialized wrapper around the more general `validated_reactive_val()` that is designed for cascading inputs. You provide it an (optionally reactive) expression for the valid `levels`, and `vrv_factor()` guarantees that its value is always one of those levels. If its current value becomes invalid (either because the `levels` change or you attempt to set an invalid value), it returns a default value (which can also be calculated via a reactive expression) instead.
...
#| components: [editor, viewer]
#| layout: vertical
webr::install("shinybatch", repos = c("https://shinybatch.shinyworks.org/", "https://shinyworks.r-universe.dev/", "https://repo.r-wasm.org/"))
